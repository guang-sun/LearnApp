...
static const CGFloat TrainPlayerAnimationTimeInterval             = 7.0f;
static const CGFloat TrainPlayerControlBarAutoFadeOutTimeInterval = 0.35f;

@interface TrainPlayerControlView () <UIGestureRecognizerDelegate>
/** 标题 */
...
@implementation TrainPlayerControlView
...
//        [self addSubview:self.closeBtn];
        ...
        // 初始化时重置controlView
        [self Train_playerResetControlView];
        // app退到后台
...
- (void)layoutSubviews
{
    ...
    [self layoutIfNeeded];
    [self Train_playerCancelAutoFadeOutControlView];
    if ( !self.isPlayEnd) {
        // 只要屏幕旋转就显示控制层
        [self Train_playerShowControlView];
    }
...
//    if ([self.delegate respondsToSelector:@selector(Train_controlView:resolutionAction:)]) {
//        [self.delegate Train_controlView:self resolutionAction:sender];
//    }
//}
...
- (void)tapSliderAction:(UITapGestureRecognizer *)tap
{
    if ([tap.view isKindOfClass:[UISlider class]]) {
        ...
        if ([self.delegate respondsToSelector:@selector(Train_controlView:progressSliderTap:)]) {
            [self.delegate Train_controlView:self progressSliderTap:tapValue];
        }
    }
}
...
//    UIInterfaceOrientation orientation = [UIApplication sharedApplication].statusBarOrientation;
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(Train_controlView:backAction:)]) {
        [self.delegate Train_controlView:self backAction:sender];
    }
...
- (void)lockScrrenBtnClick:(UIButton *)sender
{
    ...
    if ([self.delegate respondsToSelector:@selector(Train_controlView:lockScreenAction:)]) {
        [self.delegate Train_controlView:self lockScreenAction:sender];
    }
}
...
{
    sender.selected = !sender.selected;
    if ([self.delegate respondsToSelector:@selector(Train_controlView:playAction:)]) {
        [self.delegate Train_controlView:self playAction:sender];
    }
}

- (void)closeBtnClick:(UIButton *)sender
{
    if ([self.delegate respondsToSelector:@selector(Train_controlView:closeAction:)]) {
        [self.delegate Train_controlView:self closeAction:sender];
    }
}
...
{
    sender.selected = !sender.selected;
    if ([self.delegate respondsToSelector:@selector(Train_controlView:fullScreenAction:)]) {
        [self.delegate Train_controlView:self fullScreenAction:sender];
    }
}
...
- (void)repeatBtnClick:(UIButton *)sender
{
    ...
    [self Train_playerResetControlView];
    [self Train_playerShowControlView];
    if ([self.delegate respondsToSelector:@selector(Train_controlView:repeatPlayAction:)]) {
        [self.delegate Train_controlView:self repeatPlayAction:sender];
    }
}

- (void)downloadBtnClick:(UIButton *)sender
{
    if ([self.delegate respondsToSelector:@selector(Train_controlView:downloadVideoAction:)]) {
        [self.delegate Train_controlView:self downloadVideoAction:sender];
    }
}
...
{
    if ([self.delegate respondsToSelector:@selector(Train_controlView:cneterPlayAction:)]) {
        [self.delegate Train_controlView:self cneterPlayAction:sender];
    }
}
...
- (void)failBtnClick:(UIButton *)sender
{
    ...
    if ([self.delegate respondsToSelector:@selector(Train_controlView:failAction:)]) {
        [self.delegate Train_controlView:self failAction:sender];
    }
}
...
- (void)progressSliderTouchBegan:(ASValueTrackingSlider *)sender
{
    ...
    if ([self.delegate respondsToSelector:@selector(Train_controlView:progressSliderTouchBegan:)]) {
        [self.delegate Train_controlView:self progressSliderTouchBegan:sender];
    }
}

- (void)progressSliderValueChanged:(ASValueTrackingSlider *)sender
{
    if ([self.delegate respondsToSelector:@selector(Train_controlView:progressSliderValueChanged:)]) {
        [self.delegate Train_controlView:self progressSliderValueChanged:sender];
    }
}
...
{
    self.showing = YES;
    if ([self.delegate respondsToSelector:@selector(Train_controlView:progressSliderTouchEnded:)]) {
        [self.delegate Train_controlView:self progressSliderTouchEnded:sender];
    }
}
...
- (void)onDeviceOrientationChange
{
    ...
    if (TrainPlayerOrientationIsLandscape) {
        [self setOrientationLandscapeConstraint];
    } else {
        [self setOrientationPortraitConstraint];
    }
...
- (void)setOrientationLandscapeConstraint
{
    ...
    [self.backBtn setImage:TrainPlayerImage(@"TrainPlayer_back_full") forState:UIControlStateNormal];
...
//    if (self.isCellVideo) {
//        [self.backBtn setImage:TrainPlayerImage(@"TrainPlayer_close") forState:UIControlStateNormal];
...
{
    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(Train_playerHideControlView) object:nil];
    [self performSelector:@selector(Train_playerHideControlView) withObject:nil afterDelay:TrainPlayerAnimationTimeInterval];
}
...
-(void)setIslocalMovie:(BOOL)islocalMovie{
    
    _islocalMovie = islocalMovie;
    self.fullScreenBtn.hidden = islocalMovie;    
    
}

-(void)setIsFinish:(BOOL)isFinish{
    _isFinish = isFinish;
    self.videoSlider.userInteractionEnabled = isFinish;
}
...
- (UIButton *)backBtn
{
    if (!_backBtn) { 
        ...
        [_backBtn setImage:TrainPlayerImage(@"TrainPlayer_back_full") forState:UIControlStateNormal];
...
- (UIImageView *)topImageView
{
    if (!_topImageView) {
        ...
        _topImageView.image                  = TrainPlayerImage(@"TrainPlayer_top_shadow");
    }
...
- (UIImageView *)bottomImageView
{
    if (!_bottomImageView) {
        ...
        _bottomImageView.image                  = TrainPlayerImage(@"TrainPlayer_bottom_shadow");
    }
...
- (UIButton *)lockBtn
{
    if (!_lockBtn) {
        ...
        [_lockBtn setImage:TrainPlayerImage(@"TrainPlayer_unlock-nor") forState:UIControlStateNormal];
        [_lockBtn setImage:TrainPlayerImage(@"TrainPlayer_lock-nor") forState:UIControlStateSelected];
...
- (UIButton *)startBtn
{
    if (!_startBtn) {
        ...
        [_startBtn setImage:TrainPlayerImage(@"TrainPlayer_play") forState:UIControlStateNormal];
        [_startBtn setImage:TrainPlayerImage(@"TrainPlayer_pause") forState:UIControlStateSelected];
...
//        [_closeBtn setImage:TrainPlayerImage(@"TrainPlayer_close") forState:UIControlStateNormal];
...
- (ASValueTrackingSlider *)videoSlider
{
    if (!_videoSlider) {
        ...
        [_videoSlider setThumbImage:TrainPlayerImage(@"TrainPlayer_slider") forState:UIControlStateNormal];
...
- (UIButton *)fullScreenBtn
{
    if (!_fullScreenBtn) {
        ...
        [_fullScreenBtn setImage:TrainPlayerImage(@"TrainPlayer_fullscreen") forState:UIControlStateNormal];
        [_fullScreenBtn setImage:TrainPlayerImage(@"TrainPlayer_shrinkscreen") forState:UIControlStateSelected];
...
- (UIButton *)repeatBtn
{
    if (!_repeatBtn) {
        ...
        [_repeatBtn setImage:TrainPlayerImage(@"TrainPlayer_repeat_video") forState:UIControlStateNormal];
...
- (UIButton *)downLoadBtn
{
    if (!_downLoadBtn) {
        ...
        [_downLoadBtn setImage:TrainPlayerImage(@"TrainPlayer_download") forState:UIControlStateNormal];
        [_downLoadBtn setImage:TrainPlayerImage(@"TrainPlayer_not_download") forState:UIControlStateDisabled];
...
- (UIButton *)playeBtn
{
    if (!_playeBtn) {
        ...
        [_playeBtn setImage:TrainPlayerImage(@"TrainPlayer_play_btn") forState:UIControlStateNormal];
...
- (void)Train_playerModel:(TrainPlayerModel *)playerModel
{
    ...
    if (playerModel.placeholderImageURLString) {
        [self.placeholderImageView setImageWithURLString:playerModel.placeholderImageURLString placeholder:TrainPlayerImage(@"TrainPlayer_loading_bgView")];
    } else {
        self.placeholderImageView.image = playerModel.placeholderImage;
    }
    if (playerModel.resolutionDic) {
        [self Train_playerResolutionArray:[playerModel.resolutionDic allKeys]];
    }
}
...
{
    if (self.isShowing) {
        [self Train_playerHideControlView];
        return;
    }
    [self Train_playerCancelAutoFadeOutControlView];
    [UIView animateWithDuration:TrainPlayerControlBarAutoFadeOutTimeInterval animations:^{
...
- (void)Train_playerDraggedTime:(NSInteger)draggedTime totalTime:(NSInteger)totalTime isForward:(BOOL)forawrd hasPreview:(BOOL)preview
{
    ...
    if (forawrd) {
        self.fastImageView.image = TrainPlayerImage(@"TrainPlayer_fast_forward");
    } else {
        self.fastImageView.image = TrainPlayerImage(@"TrainPlayer_fast_backward");
    }
