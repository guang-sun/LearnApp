...
#import "TrainPhotoBrowserView.h"
...
#import "TrainPhotoBrowserConfig.h"
//  =============================================
#define kAnimationDuration 0.35f


@implementation TrainPhotoBrowser
{
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        self.backgroundColor = TrainPhotoBrowserBackgrounColor;
    }
    return self;
}
...
- (void)setupToolbars
{
    ...
    CGFloat  y = (kiPhoneX ) ? 70  : 30;
    indexLabel.center = CGPointMake(kAPPWidth * 0.5, y);
...
//    TrainPhotoBrowserView *currentView = _scrollView.subviews[index];
    
//    NSData * imageData = UIImageJPEGRepresentation(currentView.imageview.image,1);
//    
//    
...
//        label.text = [NSString stringWithFormat:@"%.2f K",imageData.length/1024.0];
//    
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo;
{
    ...
    [[UIApplication sharedApplication].keyWindow bringSubviewToFront:label];
    if (error) {
        label.text = TrainPhotoBrowserSaveImageFailText;
    }   else {
        label.text = TrainPhotoBrowserSaveImageSuccessText;
    }
    [label performSelector:@selector(removeFromSuperview) withObject:nil afterDelay:1.0];
}
...
- (void)setupScrollView
{
    ...
    for (int i = 0; i < self.imageCount; i++) {
        TrainPhotoBrowserView *view = [[TrainPhotoBrowserView alloc] init];
...
{
    TrainPhotoBrowserView *view = _scrollView.subviews[index];
...
- (void)layoutSubviews
{
    ...
    rect.size.width += TrainPhotoBrowserImageViewMargin * 2;
    _scrollView.bounds = rect;
    //    _scrollView.center = self.center;
    _scrollView.center = CGPointMake(self.bounds.size.width *0.5, self.bounds.size.height *0.5);
    
    CGFloat y = 0;
    __block CGFloat w = _scrollView.frame.size.width - TrainPhotoBrowserImageViewMargin * 2;
    CGFloat h = _scrollView.frame.size.height;
    
    [_scrollView.subviews enumerateObjectsUsingBlock:^(TrainPhotoBrowserView *obj, NSUInteger idx, BOOL *stop) {
        CGFloat x = TrainPhotoBrowserImageViewMargin + idx * (TrainPhotoBrowserImageViewMargin * 2 + w);
    ...
    CGFloat  yyy = (kiPhoneX ) ? 70  : 30;

    _indexLabel.bounds = CGRectMake(0, 0, 80, 30);
    _indexLabel.center = CGPointMake(self.bounds.size.width * 0.5, yyy);
...
- (void)show
{
    ...
    _contentView.backgroundColor = TrainPhotoBrowserBackgrounColor;
    ...
    [self performSelector:@selector(onDeviceOrientationChangeWithObserver) withObject:nil afterDelay:TrainPhotoBrowserShowImageAnimationDuration + 0.2];
...
-(void)onDeviceOrientationChange
{
    ...
    TrainPhotoBrowserView *currentView = _scrollView.subviews[self.currentImageIndex];
...
- (void)showFirstImage
{
    ...
    [UIView animateWithDuration:TrainPhotoBrowserShowImageAnimationDuration animations:^{
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    for (TrainPhotoBrowserView *view in _scrollView.subviews) {
...
{
    TrainPhotoBrowserView *view = (TrainPhotoBrowserView *)recognizer.view;
...
- (void)hidePhotoBrowser:(UITapGestureRecognizer *)recognizer
{
    ...
    [UIView animateWithDuration:TrainPhotoBrowserHideImageAnimationDuration animations:^{
