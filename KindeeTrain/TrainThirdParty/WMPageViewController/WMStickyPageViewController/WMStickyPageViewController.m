...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (scrollView == self.view) {
        [self wmpageScrollViewWithOff:scrollView.contentOffset];
    ...
    if (scrollView == self.basicScrollView) {
        if (self.basicScrollView.contentOffset.y >= self.maximumContentOffsetY) {
            //禁止惯性滚动
...
- (CGRect)pageController:(WMPageController *)pageController preferredFrameForContentView:(WMScrollView *)contentView {
    ...
    if (_disableSticky) {
        ...
        return CGRectMake(0, CGRectGetMaxY(preferredFrameForMenuView), CGRectGetWidth(preferredFrameForMenuView),self.view.frame.size.height - CGRectGetMaxY(preferredFrameForMenuView) - tabBarHeight - self.downInset);
        
    }else {
        
         CGFloat  float_y = self.basicScrollView.contentOffset.y ;
        NSLog(@"-----%f",float_y);
        return CGRectMake(0, CGRectGetMaxY(preferredFrameForMenuView), CGRectGetWidth(preferredFrameForMenuView),self.view.frame.size.height - CGRectGetMaxY(preferredFrameForMenuView) - tabBarHeight + float_y - self.downInset);
    }
    return CGRectZero ;
}
